<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="stylesheet" href="/audiovisual/assets/css/styleAudiovisual.css">
    <link rel="stylesheet" href="/audiovisual/assets/css/animation.css">
    <link rel="stylesheet" href="/audiovisual/assets/css/styleFonts.css">
    <link rel="stylesheet" href="/audiovisual/assets/assets/css/responsive.css">

    <title>Pixel audiovisual</title>

    <meta name="description" content="Somos Lideres en diseño y desarrollo web y marketing digital, tiendas virtuales y posicionamiento SEO, Contáctanos.">
    <meta name="keywords" content="Diseño web medellin, marketing ditial, publicidad digital, agencia digital medellin, soluciones web, páginas web medellin, redes sociales, agencia de marketing digital, agencia de marketing, seo, seo medellin, necesito una página web, ecomerce, tiendasvirutalesmedellin">

</head>
<body>
    <header>
        <div class="containerItemsHeader">
            <a href="/" class="linkLogo">
                <figure class="icono_pixel"> 
                    <img src="../assets/img/logoDrone.webp" alt="Pixel Agency">
                </figure>
            </a>
            <div class="menu_btn" id="btn_menu">
                <p>Menu</p>
                <div class="containerSimbol">
                    <span class="pixelLeftTop"></span>
                    <span class="pixelRightTop"></span>
                    <span class="pixelLeftBottom"></span>    
                    <span class="pixelRightBottom"></span>    
                </div>
            </div>
        </div>
        <div class="btnCotizacion">
            <a id="coti_proyect">Corizar Proyecto</a>
        </div>
    </header>
    <div class="menu_principal">
        <div>
            <div class="close_menu" id="close_menu">
                <span>Cerrar menú X</span>
            </div>
            <div class="container_menu_principal">
                <nav>
                    <ul>
                        <a href="../proyectos" class="cont_trabajos">
                            <div class="desc_trabajos">
                                <p>Diseño y desarrollo Productos digitales</p>
                            </div>
                            <li> 
                                <p> Proyectos </p>
                                <span> Ver mas </span>
                            </li>
                        </a>
                        <a href="../historia" class="cont_history">
                            <div class="desc_trabajos">
                                <p>Conoce quien soy y porque trabajar juntos</p>
                            </div>
                            <li> 
                                <p> Historia  </p>
                                <span> Ver mas </span>
                            </li>
                        </a>
                        <a href="../servicios" class="cont_Servicios"> 
                            <div class="desc_trabajos">
                                <p>La técnologia llegó para quedarse</p>
                            </div>
                            <li> 
                                <p> Servicios </p>
                                <span> Ver mas </span>
                            </li>
                        </a>
                        <a  class="cont_Blog">
                            <div class="desc_trabajos">

                            </div>
                            <li>
                                <p> Blog / Shop </p>
                                <span> Ver mas </span>
                            </li>
                            <span class="prox">Próximamente</span>
                        </a>
                    </ul>
                </nav>
            </div>
            <div class="containerButtons">
                <p>Latino para el Mundo con mucho ❤️</p>
                <nav class="social_media">
                    <ul>
                        <li><a href="https://www.facebook.com/pixelAgencyy/" target="_blank" class="icon-facebook"></a></li>
                        <li><a href="https://www.instagram.com/pixelagencyy/" target="_blank" class="icon-instagram"></a></li>
                        <li><a href="" class="icon-whatsapp"></a></li>
                    </ul>
                </nav>
            </div>
        </div>
    </div>
    <div class="containerPreloader">
        <figure class="logoPreloader">
            <img src="assets/img/Logo Principal.png" alt="">
        </figure>
        <span class="preload"></span>
    </div>